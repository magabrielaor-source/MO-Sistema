...
if st.session_state.rol in ['administrador', 'tecnico', 'vendedor']:
    ...
    if menu == "Dashboard":
        ...
        if not inv.empty:
            ...
            c1.metric("Inversión en Equipos", f"${inv['Costo_Total_Real'].sum():,.2f}")
        ...
        with st.expander("➕ Registrar Equipo Nuevo (Codificado)"):
            with st.form("f_inv"):
                c1, c2, c3 = st.columns(3)
        ...
        with st.form("f_v"):
            ...
            c_v = st.number_input("Costo Inversión Total $")
        ...
        st.header("📝 Informe Técnico Detallado")
        ...
        with st.form("f_inf"):
            ...
            st.markdown('<div style="display:flex; justify-content:space-around; margin-top:30px;"><p>__________________<br>Firma Técnico</p><p>__________________<br>Firma Cliente</p></div>', unsafe_allow_html=True)
